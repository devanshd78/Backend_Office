...
class InvoicePDF(FPDF):
    ...
    def footer(self):
        ...
        if hasattr(self, 'invoice_data') and self.invoice_data:
            contact = (
                f"{self.invoice_data.get('company_email', '')} | "
                f"{self.invoice_data.get('company_phone', '')} | "
                f"{self.invoice_data.get('website', '')}"
            )
