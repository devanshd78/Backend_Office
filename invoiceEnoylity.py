...
def generate_invoice_number():
    ...
    last = db.invoiceEnoylity.find({"invoice_number": {'$regex': '^INV\d{5}$'}}) 
...
def generate_invoice_route():
    try:
        ...
        db.invoiceEnoylity.insert_one(rec)
        # send PDF
...
def get_invoice_list():
    try:
        ...
        invoices_cursor = db.invoiceEnoylity.find(filter_criteria).skip(skip).limit(per_page)
        ...
        total_invoices = db.invoiceEnoylity.count_documents(filter_criteria)
