...
def login_combined():
    try:
        ...
        email_or_username = input_data.get('email')  # Admin uses email, Subadmin uses username
        ...
        # === Try Admin Login ===
        admin_query = {'email': {'$regex': f'^{re.escape(email_or_username)}$', '$options': 'i'}}
        admin = db.admin.find_one(admin_query)
        ...
        # === Try Subadmin Login ===
        subadmin = db.subadmin.find_one({'username': email_or_username})
        if subadmin and check_password_hash(subadmin['password_hash'], password):
            subadmin.pop('_id', None)
            subadmin.pop('password_hash', None)
            subadmin['role'] = "subadmin"
            return format_response(True, "Subadmin login successful", subadmin, 200)

        return format_response(False, "Invalid credentials", None, 404)
